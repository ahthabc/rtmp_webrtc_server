...
func (enc *Encoder) Init(sample_rate int, channels int, application Application) error {
	...
	if cerror != C.OPUS_OK {
		return errors.New("failed to create encoder engine")
	}
...
func (enc *Encoder) Encode(pcm []int16, data []byte) (int, error) {
	...
	// int n = opus_encode(
	// 	encoder,
	// 	(const opus_int16*)(pcm_buf),
	// 	ret,
	// 	audioinfo.a,
	// 	MAXBUFFER_OPUSAUDIO_DATA
	// );
...
// func (enc *Encoder) Opusencodectrl()error{
// 	// printf("opusencodectrl start \r\n");
// 		// opus_encoder_ctl(encoder, OPUS_SET_BITRATE(BITRATE * 1000));
// 		 //   printf("set encoder COMPLEXITY %d\r\n",9);
...
// 	 C.opus_encoder_ctl(enc.p, C.OPUS_SET_SIGNAL(C.OPUS_AUTO));//信号signal_type要么OPUS_AUTO（默认），OPUS_SIGNAL_VOICE或OPUS_SIGNAL_MUSIC
// 	   // opus_encoder_ctl(encoder, OPUS_SET_BANDWIDTH(BANDWIDTH));
// 	 // opus_encoder_ctl(encoder, OPUS_SET_VBR(VBR));

// 	 // opus_encoder_ctl(encoder, OPUS_SET_SIGNAL(OPUS_SIGNAL_VOICE));

// 	 opus_encoder_ctl(encoder, OPUS_SET_BITRATE(OPUS_AUTO));     ///使用AUTO主要是应为视频会议过程中,大部分场景是不说话的,减少带宽占用
...
// 	 opus_encoder_ctl(encoder, OPUS_SET_VBR_CONSTRAINT(0));//0:Unconstrained VBR., 1:Constrained VBR.

// 	 opus_encoder_ctl(encoder, OPUS_SET_COMPLEXITY(5));//range:0~10

// 	 opus_encoder_ctl(encoder, OPUS_SET_FORCE_CHANNELS(CAPTURE_CHANNELS)); //1:Forced mono, 2:Forced stereo

// 	 opus_encoder_ctl(encoder, OPUS_SET_APPLICATION(OPUS_APPLICATION_VOIP));//

// 	 opus_encoder_ctl(encoder, OPUS_SET_INBAND_FEC(1));//0:Disable, 1:Enable

// 	 opus_encoder_ctl(encoder, OPUS_SET_EXPERT_FRAME_DURATION(OPUS_FRAMESIZE_20_MS));
// 		printf("opusencodectrl ok \r\n");
